...
const serviceNameMapping = new Map<string, ServiceInfo>([
  ...
  addServiceMapping(hapNodeJs.Service.MotionSensor, 'sensors.md'),
...
`;
  ...
  if (device.whiteLabel) {
    ...
    devicePage += `\n## White-label models\n${device.whiteLabel.map((d) => `* [${d.vendor}](../index.md#${normalizeNameForAnchor(d.vendor)}) ${d.model}`).join('\n')}\n`;
  }
...
${vendors.filter(v => v.substr(0, 1).toUpperCase() === letter).map(v => `<span class="vendor">[${v}](index.md#${normalizeNameForAnchor(v)})</span>`).join('\n')}
<div style="clear:both" />
